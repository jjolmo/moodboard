...
const GITHUB_REPO: &str = "jjolmo/moodboard";
...
pub async fn check_for_updates() -> Result<UpdateCheck, String> {
    ...
    let client = reqwest::Client::builder()
        .user_agent("SimpleMoodboard-Updater")
        .build()
...
#[tauri::command]
pub async fn download_and_apply_update(download_url: String) -> Result<String, String> {
    #[cfg(target_os = "linux")]
    {
        let current_exe = std::env::current_exe()
            .map_err(|e| format!("Can't find current executable: {}", e))?;

        if current_exe.to_string_lossy().contains("AppImage") || std::env::var("APPIMAGE").is_ok() {
            ...
            return Ok("Update applied! Restart the app.".to_string());
        }
    }
